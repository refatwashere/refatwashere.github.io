...
2. Verify `mom.html` links to `Tradejournal.html`.
3. Verify `resources.html` downloads all 3 `.txt` files.
4. Verify `projects.html` renders SVG placeholders.
5. Verify `Tradejournal.html` renders `<script>` as text.
6. Verify `crypto/crypto.html`:
- chart/ticker/watchlist/alerts work
- settings persist
- no console errors
