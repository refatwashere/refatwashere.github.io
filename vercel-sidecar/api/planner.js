...
module.exports = (req, res) => {
  ...
  if (expectedToken && req.headers['x-planner-token'] !== expectedToken) {
...
function calculateConfidence(r) {
  ...
  if (r.marketPrice && r.limitPrice) {
    ...
    if (spread < 0.01) confidence += 0.1;
  }

  if (r.size < 1) confidence += 0.05;
...
function calculateDexConfidence(r, tokenInKnown, tokenOutKnown) {
  ...
  let confidence = (tokenInConfidence + tokenOutConfidence) / 2;
